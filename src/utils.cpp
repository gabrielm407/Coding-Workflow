...
bool runSelfTest() {
  ...
  // calculator tests
  if (Calculator::add(4, 1) != 5) {
    std::cout << "[self-test] calc add failed\n";
    ok = false;
  }
  if (Calculator::sub(5, 2) != 3) {
    std::cout << "[self-test] calc sub failed\n";
    ok = false;
  }
  if (Calculator::mul(3, 4) != 12) {
    std::cout << "[self-test] calc mul failed\n";
    ok = false;
  }
  try {
    double v = Calculator::div(10, 2);
    if (v != 5.0) {
      std::cout << "[self-test] calc div result wrong\n";
      ok = false;
    }
  } catch (...) {
    std::cout << "[self-test] calc div threw unexpectedly\n";
    ok = false;
  }
